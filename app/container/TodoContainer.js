...
class TodoContainer extends React.Component{
        ...
        if(lastUserState!==''){
            ...
            apiAuth.onGetUser().then((res)=>{
                if(res.data.response){
                    ...
                    apiTODO.onGetTodo(res.data.response._id)
                    .then((mytodo)=>{
                        this.setState({
                    ...
                    apiTODO.onGetCompleted(res.data.response._id)
                    .then((mytodo)=>{
                        this.setState({
    ...
    handleOnComplete(todo,index){
        ...
        apiTODO.onEdit(todo._id,"isCompleted",!todo.isCompleted)
            .then(res=>{
                if(res.data.success){
    ...
    OnDelete(todo,index){
        ...
        let lastItemState = this.state.items;
        apiTODO.onDelete(todo._id).then(res=>{
            if(res.data.success){
    ...
    DelAllComplete(){
        this.setState({isUpdating:true});
        apiTODO.onDelAllComplete(this.state.user)
        .then(res=>{
            if(res.data.success){
                console.log("success deleting all completed todo")
            }
            apiTODO.onGetTodo(this.state.user)
            .then(mytodo=>{
            console.log(mytodo)
            this.setState({isUpdating:false,
                        items: [...mytodo]});
    ...
    todoOpen(){
        ...
        apiTODO.onGetOpen(this.state.user)
        .then(mytodo=>{
            this.setState({isUpdating:false,
                        items: [...mytodo]}
                        );
    ...
    todoCompleted(){
        ...
        apiTODO.onGetCompleted(this.state.user)
        .then(mytodo=>{
            this.setState({isUpdating:false,
                        items: [...mytodo]});
        }).catch(err=>{
